...
async fn handle_connection_inner(
    ...
    let selected_provider_id = provider.id.clone();
    ...
    let (mut upstream, mut turn_state, initial_accounting, mut turn_started, mut timeout_config) = loop {
        let initial_accounting = loop {
            ...
            .await
            {
                ...
                {
                    provider = next_websocket_provider(
                        &turn_providers,
                        &mut provider_index,
                        provider_attempts,
                ...
                match downstream_message {
                    DownstreamMessage::Text(text) => {
                        ...
                        let (text, next_state, already_sent, next_turn_started) =
                            ...
                            if next_ctx.current_provider_id == provider.id {
                            ...
                            retain_fallback_provider =
                                provider.id != next_ctx.current_provider_id;
                ...
                match upstream_message {
                    UpstreamMessage::Text(text) => {
                        ...
                        }) && !relayed_response_event
                        {
                            ...
                            let mut failed_usage = terminal_data
                                .as_ref()
                            ...
                            while allow_provider_fallback
                                && provider_index + 1 < turn_providers.len()
                                && provider_attempts < provider_attempt_limit
                            {
                                ...
                                if let Some((status_code, event)) = failed_usage.as_ref() {
                                    spawn_codex_websocket_usage(
                                        state,
                                        &provider,
                                        &turn_state,
                                ...
                                provider = candidate;
                                retain_fallback_provider =
                                    provider.id != turn_context.current_provider_id;
